...
function buildMessageRow(msg) {
  ...
  const svc      = msg.service ? `<span class="msg-service-tag">${serviceLabel(msg.service)}</span>` : '';
...
function deleteAllQuotes() {
  showConfirm('Delete all quote requests? This cannot be undone.', () => {
    const quoteIds = allMessages.filter(m => m.service && m.service !== '').map(m => m.id);
...
function confirmDelete(id) {
  showConfirm('Are you sure you want to delete this message?', () => deleteMessage(id));
}
